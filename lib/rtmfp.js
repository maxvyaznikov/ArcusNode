...
var rtmfp = require('../build/Release/rtmfp');
...
RTMFP_VALID_MARKERS = [0x8d, 0x8e, 0x8a, 0x0d, 0x0b, 0x89, 0x09, 0x49, 0x4e, 0x4a, 0x4d, 0x0e, 0x0a];
RTMFP_VALID_MESSAGES = [0x10, 0x11, 0x30, 0x38, 0x51, 0x01, 0x41, 0x0c, 0x4c, 0x18, 0x71, 0x70, 0x78, 0x5e];

RTMFP_MSG_EMPTY = 0x00;
RTMFP_MSG_AMF_WITH_HANDLER = 0x14;
RTMFP_MSG_AMF_METHOD = 0x11;
RTMFP_MSG_AMF = 0x0f;
RTMFP_MSG_AUDIO = 0x08;
RTMFP_MSG_VIDEO = 0x09;
RTMFP_MSG_RAW_DATA = 0x04;
...
var RTMFP = module.exports = function(){
    ...
    switch(type)
    {
      ...
      case 0x71:
        ...
        while(clippedPkt.available() > 0 && clippedPkt.readInt8() == 1) {
          message.endpoints.push(this.readRawIpv4(clippedPkt));
        }
        break;      
      ...
      case 0x11 :
        ...
        if(type == 0x11 && lastMessage != null) {
          ...
          message.stage = lastMessage.stage + 1;
          message.delta = lastMessage.delta + 1;
        } else {
          message.flow = clippedPkt.readInt8();
          message.stage = clippedPkt.readInt8();
        ...
        if(message.flag == 0x80 || message.flag == 0x83) {
          var headPartSize = 0;
          message.header = [];
          while((headPartSize = clippedPkt.readInt8()) != 0){
            message.header.push(clippedPkt.readBytes(headPartSize, true));
          }
        }

        //Flag 0x03 is connect failed answer/retry without header (when connection request was acknowledged)
        //Flag 0x83 is connect failed answer/retry WITH header (if connection request was NOT acknowledged)
        if(message.flow == 0x02 && (message.flag == 0x83 || message.flag == 0x03)){
          message.type = RTMFP.MESSAGE_RESEND;
          return message;
        }
        
        //Handle Flow 0x02 (Method)
        if(message.flow == 0x02)
        {
          //Read Message Type
          var messageType = clippedPkt.readInt8();
          switch(messageType){
            // 0x14
            case RTMFP_MSG_AMF_WITH_HANDLER:
              message.handler = clippedPkt.readInt32();
              message.commandName = AMF0.readString(clippedPkt);
              message.commandHandle = AMF0.readNumber(clippedPkt);
              break;
            case RTMFP_MSG_AMF_METHOD:
              message.handler = clippedPkt.readInt32();
              clippedPkt.skip(1); //0x00
              message.commandName = AMF0.readString(clippedPkt);
              message.commandHandle = AMF0.readNumber(clippedPkt);
              break;
              
            // Yet unhandled types
            case RTMFP_MSG_RAW_DATA:
            case RTMFP_MSG_EMPTY:
            case RTMFP_MSG_AMF:
            case RTMFP_MSG_AUDIO:
            case RTMFP_MSG_VIDEO:
              throw new Error('Unhandled message type ', messageType);
              break;
            //Exit with error on unrecognized message type
            default:
          ...
          switch(message.commandName) {
            ...
            //Handle endpoints for NetConnection
            case 'setPeerInfo':
              message.type = RTMFP.NET_CONNECTION_ADDRESSES;
              
              clippedPkt.skip(1);
              
              //TODO: read AMF0 strings and handle as endpoints outside
              message.endpoints = [];
              while(clippedPkt.available() > 3 && clippedPkt.readInt8() == 0x02)
              {
                message.endpoints.push(this.readAddress(clippedPkt));
              }
        ...
        else if(message.flow > 0x02 && message.stage == 0x01)
        {
          ...
          clippedPkt.skip(1); //0x01
          var gidLength = clippedPkt.readU29();
          message.groupId = clippedPkt.readBytes(gidLength, true);
        }
  ...
  this.writePacket = function(pkt, message){
    ...
    switch(message.type)
    {
      ...
      case RTMFP.NET_CONNECTION_REQUEST:
        ...
        break;
      //
      // NC_SUCCESS, NC_REJECTED
      //
      case RTMFP.NC_REJECTED_RESPONSE:
      case RTMFP.NC_SUCCESS_RESPONSE:
        ...
        //Write header flag
        if(typeof message.header !== 'undefined' && Array.isArray(message.header)){
          pkt.writeInt8(0x80);
        } else if(typeof message.header === 'undefined'){
          pkt.writeInt8(0x00);
        } else {
          throw new Error('Message header needs to be an array with header values');
        }
        
        pkt.writeInt8(message.flow);
        pkt.writeInt8(message.stage);
        pkt.writeInt8(0x01); //TODO: stage of the last flow message that was sent
        
        //Write Header 
        if(message.flag == 0x80 || message.flag == 0x83) {
          for(var hp = 0; hp < message.header.length; hp++){
            pkt.writeInt8(message.header[hp].length);
            pkt.writeBuffer(message.header[hp]);
          }
          //Close Header
          pkt.writeInt8(0x00);
        }
        ...
        //Write Message Type 
        pkt.writeInt8(RTMFP_MSG_AMF_WITH_HANDLER);
        pkt.writeInt32(0);
        ...
        //Write rejected or success status object
        var responseObject = {
          objectEncoding: 3, 
          level: 'status'
        };
        
        if(message.type == RTMFP.NC_REJECTED_RESPONSE){
          responseObject.description = message.description || 'Connection rejected';
          responseObject.code = 'NetConnection.Connect.Rejected';
        } else if(message.type == RTMFP.NC_SUCCESS_RESPONSE){
          responseObject.description = message.description || 'Connection succeeded';
          responseObject.code = 'NetConnection.Connect.Success';
        }
        AMF0.writeObject(pkt, responseObject);
      ...
      case RTMFP.RENDEZVOUZ_RESPONSE:
        ...
        for(var i = 0; i < message.endpoints.length; i++)
        {
          ...
          this.writeAddress(pkt, message.endpoints[i], publicFlag);
          publicFlag = false;
        }
  ...
  this.writeAddress = function(pkt, endpoint, isPublic) {
    //validate endpoints (has address and port)
  ...
  this.createClientNonce = function(certificate){
    if(!certificate || certificate.length != 64){
...
RTMFP.NC_SUCCESS_RESPONSE = 0x0C;
...
RTMFP.MESSAGE_RESEND = 0x19;
RTMFP.NC_REJECTED_RESPONSE = 0x1A;
